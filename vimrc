...
autocmd FileType c,cpp set noexpandtab shiftwidth=4 tabstop=4
...
nnoremap <C-A> :GeneratePerlAccessor<CR>
...
function! GeneratePerlAccessor()
  let Var = <cword>
  echo "sub " & Var & " {\n"
  echo "\tmy $self = shift();\n"
  echo "\t$self->{" & Var & "} = shift() if(@_);\n"
  echo "\treturn $self->{" & Var & "};\n"
  echo "}\n"
endfunction
